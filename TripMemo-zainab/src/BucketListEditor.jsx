...
export default function BucketListEditor({ bucketList, onBack }) {
  ...
  const fetchItems = async () => {
    try {
      const res = await fetch(`http://localhost:5000/bucket-lists/${bucketList.id}/items`, {
        credentials: "include"
      });
...
const addStickerItem = async (sticker) => {
  ...
  const updateTextItem = async (id, content) => {
    ...
    try {
      await fetch(`http://localhost:5000/bucket-lists/items/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  try {
    await fetch(`http://localhost:5000/bucket-lists/items/${id}/check`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
